...
namespace StardustOS.SDSystem
{
    public static class BitmapUtils
    {
        ...
        public static void DrawFilledRectangle(this Bitmap bmp,Color col,int x,int y,int w,int h)
        {
        ...
        public static void DrawImage(this Bitmap bmp, Image img, int x, int y)
        {
            ...
            for (int X = x; X < x + img.Width; X++)
            {
                for (int Y = y; Y < y + img.Height; Y++)
                {
                    ...
                    bmp.SetPixel(Color.FromArgb(img.RawData[(X-x) + ((Y-y)*img.Width)]), X, Y);
        ...
        // Ripped straight from https://github.com/CrystalOSDevelopment/CrystalOS_2.0/blob/main/CrystalOS2/ImprovedVBE.cs. Later i will implement CosmosTTF
        public static void _DrawACSIIString(this Bitmap bmp,Color color,string s, int x, int y)
        {
                    ...
                    for (int i = 0; i < 16; i++)
                    {
                        for (int j = 0; j < 8; j++)
                        {
                            if ((fontbuf[i] & (0x80 >> j)) != 0)
                            {
                                ...
                                {
                                    bmp.SetPixel(color,(int)((x + j) + (c * 8)), (int)(y + i + (l * 16)));
                                }
                            }
                        }
                    }
